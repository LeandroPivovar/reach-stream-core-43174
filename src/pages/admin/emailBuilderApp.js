...
function showToast(message) {
  const toast = document.getElementById('email-builder-toast');
  const msgEl = document.getElementById('email-builder-toast-message');
...
function renderIconLibraryGrid(searchTerm = '', category = 'all') {
  ...
  // Exposed to React so handleLoad can restore a template with all event listeners intact
  window.loadCanvasHTML = function(html) {
    const canvas = document.getElementById('email-canvas');
    if (!canvas) return;
    ...
    canvas.querySelectorAll('.email-element').forEach(block => {
      ...
      const editable = block.querySelector('.inline-edit');
      if (editable) editable.addEventListener('blur', () => saveState());
    });
