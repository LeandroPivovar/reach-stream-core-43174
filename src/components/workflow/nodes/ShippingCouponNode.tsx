...
interface ShippingCouponNodeData {
  label?: string;
  code?: string;
...
export const ShippingCouponNode: React.FC<{ data: ShippingCouponNodeData }> = ({ data }) => {
  ...
  const isConfigured = minPurchaseValue && expirationDays;
      ...
      <Card className="w-64 p-4 bg-gradient-to-br from-blue-500/10 to-blue-600/10 border-blue-500/30 shadow-lg">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">Shopify / LI / Nuvem</p>
          </div>
          <div className="flex gap-1">
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Configurar Frete Grátis</DialogTitle>
                </DialogHeader>

                <div className="space-y-4 py-4">
                  <div className="grid gap-2">
                    <Label htmlFor="coupon-code">Prefixo/Código do Cupom</Label>
                    <Input
                      id="coupon-code"
                      value={code}
                      onChange={(e) => setCode(e.target.value)}
                      placeholder="Ex: FRETEGRATIS (opcional)"
                    />
                    <p className="text-[10px] text-muted-foreground">Se vazio, um código aleatório será gerado.</p>
                  </div>
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button onClick={handleSave} disabled={!minPurchaseValue || !expirationDays}>
                    Salvar
                  </Button>
                </div>
              </DialogContent>
        ...
        {isConfigured ? (
          <div className="mt-3 p-3 bg-background/50 rounded-md border space-y-1">
            <div className="flex justify-between text-xs">
              ...
              <span className="font-medium text-blue-600 font-bold">{code || 'Aleatório'}</span>
            </div>
            <div className="flex justify-between text-xs">
          ...
          <div className="mt-3 p-3 bg-background/50 rounded-md border">
            <p className="text-xs text-muted-foreground">
              Clique no ícone para configurar o frete grátis
            </p>
          </div>
        )}
      </Card>
